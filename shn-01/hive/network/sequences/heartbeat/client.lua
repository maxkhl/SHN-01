-- Client-side heartbeat sequence function
...
function sendHeartbeat()
    if hive.address and not bootstrap then
        mdm.send(hive.address, 2022, "heartbeat", "HEARTBEAT", node.id)
    end
end
...
function relayToNode(targetNodeId, message)
    if hive.address and not bootstrap then
        mdm.send(hive.address, 2022, "heartbeat", "RELAY", targetNodeId, message, node.id)
    end
end
