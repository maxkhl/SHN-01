...
return function(hive, protocol)
    return function(message)
        ...
        debugPrint("Disconnect request from node <c=0xFFFFFF>" .. tostring(nodeId) .. "</c> at <c=0x888888>" .. tostring(message.remoteAddress) .. "</c>")
        
        -- Remove node from connected nodes
        local node = hive.nodes[message.remoteAddress]
        if node then
            node:remove()
            print("<c=0xFFAA00>● Node disconnected:</c> <c=0xFFFFFF>" .. tostring(nodeId) .. "</c> <c=0xFFAA00>(graceful)</c>")
        else
