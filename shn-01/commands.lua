...
console:addCommand("COMPUTER.INFO.DEVICES","Returns the computers device infos",function()
...
console:addCommand("HDD.SPACE", "Returns space information about the OS hard drive", function()
  ...
  console:horizontalGraph(spaceUsed, spaceTotal)
  print("Free: " .. tostring(spaceTotal - spaceUsed) .. " bytes")
  console:horizontalGraph(spaceTotal - spaceUsed, spaceTotal)

end)

console:addCommand("COMPONENT.LIST","Returns the computers components (use parameter as name filter)",function(filter)
  for k, v in pairs(component.list(filter)) do
      console:log(v .. ": " .. k)
  end
end)
console:addCommand("COMPONENT.FIELDS","Returns the components fields. Expects an address",function(address)
    for k, v in pairs(component.proxy(address)) do
        console:log(k .. ": " .. tostring(v))
    end
end)

console:addCommand("GLOBALS", "Shows all current global attributes and fields", function()
    for k, v in pairs(_G) do
        console:log(k .. ": " .. tostring(v))
    end
end)

console:addCommand("BUFFER.REQUIRE", "Shows the content of the require buffer", function()
